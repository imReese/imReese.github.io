...
<LayerDiagram
  title="HiCache 写路径不是一次 set，而是两级备份"
  ...
  layers={[
    ...
    {
      label: 'L3：存储后端',
      detail: 'Mooncake 后端根据 page key 生成并写入 Store 对象',
      tone: 'mauve',
...
<FlowDiagram
  ...
  caption="这条路径把 GPU 上已经生成的 KV Cache 逐级备份，但每一级成功都只能证明该层的数据已经就绪。"
  steps={[
    {
      label: '已生成的 GPU KV',
      meta: 'node.value',
...
<SequenceDiagram
  title="write_backup 把一个基数树节点从 GPU 备份到主机内存"
