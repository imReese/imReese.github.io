...
<FlowDiagram
  ...
  steps={[
    ...
    },
    {
      label: '探测存储层',
      meta: 'batch_exists',
      detail: '按页键查询连续可用边界，混合缓存还要合并辅助内存池。',
      tone: 'blue',
...
v1 会把每个逻辑页键展开成 Mooncake 对象键，再从主机内存池取得目标缓冲区指针：
...
返回结果是对象级的。Mooncake 后端再把对象级结果聚合为页级布尔值：
...
这解释了为什么一个页键背后可能有多个 Mooncake 对象。HiCache 只关心整页是否可用；Mooncake Store 看到的则是更细粒度的对象。
...
<SequenceDiagram
  ...
  steps={[
    ...
    },
    {
      ...
      detail: '按页键查询连续命中边界，并把对象结果聚合为页级结果',
      tone: 'blue',
    },
    {
      from: '存储后端',
