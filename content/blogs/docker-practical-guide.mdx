...
description: '一份 Docker 工具介绍和常用命令手册：覆盖 image、container、run、logs、exec、volume、network、compose 和常见排障。'
author: 'Reese'
date: '2024-08-18'
topics:
  - cloud-tooling
...
Docker 的关键是分清“镜像”和“容器”：镜像负责交付一致性，容器负责运行状态。排障时先看 `ps -a`、`logs`、`inspect`，数据持久化先确认 volume，清理命令动手前先确认不会删掉重要状态。
