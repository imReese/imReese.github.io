---
title: 'SGLang v0.5.14 接入 Mooncake Store：缓存页标识、零拷贝与共享 Transfer Engine'
description: '沿着一次 KV Cache 的写入与读回，说明 SGLang 如何根据缓存页标识生成 Mooncake Store 对象，以及主机端 KV 缓冲区为何是两套系统的数据交接点。'
author: 'Reese'
date: '2026-06-30'
topics:
  - llm-serving
...
<FlowDiagram
  ...
  caption="这张图只表达本文覆盖的源码路径：page key 和主机端 KV 缓冲区怎样跨过 HiCacheStorage 边界。"
  steps={[
    {
      label: '推理引擎中的缓存生命周期',
    ...
    },
    {
      ...
      detail: 'L3 后端接收 page key、主机页索引和可选的 PoolTransfer。',
      tone: 'blue',
    ...
    {
      label: '可观察结果',
      meta: '主机端缓存页已填充 / Store 对象已写入',
      detail:
        '这一步仍不代表数据已被 GPU 侧计算复用；还要确认 HiCache 回载和调度器状态。',
      tone: 'green',
      items: ['主机端 KV Cache 页', '尚未回载至 GPU'],
    },
  ]}
/>
...
<SequenceDiagram
  ...
  caption="SGLang 向后端传入 page key 和主机端缓冲区信息；Mooncake Store 负责对象键、元数据和零拷贝读写。"
  ...
  steps={[
    {
      ...
      detail: 'page key 是 token 页的滚动哈希结果，缓冲区来自主机 KV 池',
      tone: 'teal',
    },
    {
      from: 'HiCacheStorage',
    ...
    },
    {
      from: 'Mooncake 后端',
      to: 'Mooncake Store',
      label: 'Store 对象键 + 缓冲区指针',
      detail: '根据 MHA、MLA 或混合缓存布局生成对象键，并调用零拷贝 API',
      tone: 'mauve',
    },
  ]}
/>
...
<ExpandableNotes
  ...
  notes={[
    ...
    },
    {
      ...
      tone: 'teal',
      items: ['page key', '主机页索引', 'PoolTransfer'],
    },
    {
...
<MappingDiagram
  title="一个 page key 会按模型布局映射为多个 Store 对象"
  ...
  groups={[
    ...
    {
      label: '注意力头拆分 / 混合内存池',
      detail: '同一页可能继续映射到多个 rank 或辅助状态',
      tone: 'amber',
...
这解释了为什么 Mooncake 后端中有 `_get_mha_buffer_meta`、`_get_mla_buffer_meta`、`_get_mha_split_heads_buffer_meta` 等函数。它们并非装饰性代码，而是把 SGLang 的主机 KV 布局转换为 Mooncake Store 所需的对象键列表和缓冲区指针列表。
...
<SequenceDiagram
  ...
  caption="无论是 batch_put_from 还是 batch_get_into，Mooncake 后端处理的都是主机端缓冲区与对象键之间的对应关系。"
  actors={[
    'GPU KV 槽位',
    '主机端 KV Cache 页',
    'Mooncake 后端',
  ...
  steps={[
    {
      ...
      to: '主机端 KV Cache 页',
      label: 'write_backup',
      detail: 'SGLang 先把 GPU 中的 KV Cache 备份到主机内存',
      tone: 'teal',
    },
    {
      from: '主机端 KV Cache 页',
