  ...
  groups={[
    {
      ...
      tone: 'mauve',
      items: ['预填充工作进程', '解码工作进程', '共享 Transfer Engine', 'KV 传输'],
    },
    {
      label: 'SGLang HiCache L3',
      detail: '前缀页写入远端缓存',
      tone: 'teal',
      items: ['HiCacheStorage', '主机 KV 池', 'batch_get_into', 'batch_put_from'],
    },
    {
      label: 'vLLM / LMCache',
      detail: '缓存后端 / 对象存储',
...
Python 绑定把 C++ Store 和 Transfer Engine 暴露给 Python；张量和对象包装层把上层对象翻译成 Store 可处理的组成部分与切片；SGLang 更关注预填充和解码分离场景下的 KV 传输；LMCache/vLLM 更容易从缓存后端和对象存储的角度接入。
